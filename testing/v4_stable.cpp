...
#define ATTACK_THRESHOLD 55 // Lowered from 40 to require better alignment (centering)
...
#define ESCAPE_DURATION 800  // ms for forward burst after spin
...
void loop() {
  ...
  if (dangerlevel > 0 || defenceSubMode != 0 || previous_mode == DEFENCE_MODE 
    && millis() - defenceStartTime < (200 + SPIN_DURATION + ESCAPE_DURATION)) { // Any sensor black, edge detected
    mode = DEFENCE_MODE;
  ...
  if (mode == ATTACK_MODE) {
    ...
    if (Line_Sensor_1 == LINE_SENSOR_BLACK || defenceSubMode != 0) {  // Continue sequence even if sensor clears
        if (defenceSubMode == 0) {  // Start action
            defenceSubMode = 3;  // Stop/brake phase to halt momentum
            defenceStartTime = currentTime;
            ADirection = MOTOR_BACKWARDS;  // Brief backward for active braking
            BDirection = MOTOR_BACKWARDS;
            AMotorDuty = 150;  // Low duty for gentle brake (tune if needed)
            BMotorDuty = 150;
        } else if (defenceSubMode == 3 && currentTime - defenceStartTime >= 200) {  // 200 ms stop (tune as needed)
            defenceSubMode = 1;  // Spin phase
            defenceStartTime = currentTime;
            ADirection = MOTOR_FORWARDS;  // Spin in place (left forward, right back) - swap if wrong direction
            BDirection = MOTOR_BACKWARDS;
            AMotorDuty = 255;  // Max for sharp spin
            BMotorDuty = 255;
            ...
            AMotorDuty = 0;  // Ensure stop after sequence
            BMotorDuty = 0;
        }
    }
    // Sides: Differential turns (only if not in front sequence)
    ...
    unsigned long currentTime = millis();
    AMotorDuty = 0; // Reset duties to prevent carryover
    BMotorDuty = 0;
