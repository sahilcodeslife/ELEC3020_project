...
#define BLIND_MODE 4
#define ATTACK_SEARCH_SUBMODE 5  // New sub-mode for oscillating scan during attack
#define MOTOR_FORWARDS 0  // Added: Define forward motor direction
#define MOTOR_BACKWARDS 1  // Added: Define backward motor direction
...
#define MAX_SCAN_CYCLES 1  // Number of left/right oscillations before falling back to SEARCH_MODE
...
unsigned long scanStartTime = 0;  // New: Timer for scan turns
int scanDirection = 0;  // New: 0 = left (counter-clockwise), 1 = right (clockwise)
int scanCycleCount = 0;  // New: Track oscillations to limit them
//-------------------------------------------------------------------------------------
...
void loop() {
  ...
  int previous_mode = mode;  // Track for transition to submode
  ...
  if (dangerlevel > 0) { // Any sensor black, edge detected
    ...
    // else stay in submode (sub block will handle snap-back or fallback)
    ...
    scanCycleCount = 0;  // Reset scan counter
  } else {
    mode = SEARCH_MODE; // Default to SEARCH_MODE if no black lines and opponent not close enough
  }

  // Trigger submode on transition from attack (e.g., opponent dodged)
  if (previous_mode == ATTACK_MODE && mode == SEARCH_MODE) {
    mode = ATTACK_SEARCH_SUBMODE;
    scanStartTime = millis();
    scanDirection = 0;  // Start with left turn
    scanCycleCount = 0;
  }
  ...
  if (mode == ATTACK_MODE) {
    ...
    if (dist1 <= ATTACK_THRESHOLD) {
      // Opponent detected during scan: snap back to ATTACK_MODE
      mode = ATTACK_MODE;
      lastAttackCheck = millis();  // Reset timer
      scanCycleCount = 0;
    } else if (scanCycleCount >= MAX_SCAN_CYCLES * 2) {  // After full cycles (left+right pairs), fall back
      mode = SEARCH_MODE;
      scanCycleCount = 0;
    } else {
      ...
      if (currentTime - scanStartTime >= SCAN_TURN_DURATION + SCAN_PAUSE_DURATION) {
        ...
        scanDirection = 1 - scanDirection;  // Toggle 0<->1
        scanCycleCount++;
      }
    }
  }
...
void drawUI(int Line_Sensor_1, int Line_Sensor_2, int Line_Sensor_3, int Line_Sensor_4, int Line_Sensor_5,
  ...
  switch (mode) {
    ...
    case ATTACK_SEARCH_SUBMODE: modeStr = "SEAR2"; break;
